...
static const Type error, integer(INT), voidPointer(VOID, 1);
 ...
 *
 * Description:	Check a sizeof expression.  The type of the expression
...
Expression *checkSizeof(Expression *expr)
{
    ...
    if (t != error)
	if (t.isPredicate() && (t.specifier() != VOID || t.indirection() > 0))
 ...
 *		must be identical and predicate types after promotion, and
 ...
 *
 *		int x int
 *		pointer(T) x pointer(void)
 *		pointer(void) x pointer(T)
 */
